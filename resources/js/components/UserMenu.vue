  ...
  <Menu as="div" class="relative inline-block text-left">
    ...
    >
      <span class="relative inline-flex size-9 items-center justify-center overflow-hidden rounded-full bg-gradient-to-br from-indigo-500 to-sky-500 text-sm font-semibold uppercase text-white ring-2 ring-white dark:ring-gray-900">
        <img v-if="props.user.avatar" :src="props.user.avatar" alt="" class="absolute inset-0 size-full object-cover" />
    ...
    <TransitionRoot as="template" appear>
      <TransitionChild as="template" enter="transition ease-out duration-150" enter-from="opacity-0 translate-y-1" enter-to="opacity-100 translate-y-0" leave="transition ease-in duration-100" leave-from="opacity-100 translate-y-0" leave-to="opacity-0 translate-y-1">
        <MenuItems class="absolute right-0 mt-2 w-56 origin-top-right overflow-hidden rounded-2xl bg-white p-1 text-sm shadow-lg ring-1 ring-black/5 focus:outline-none dark:bg-gray-900 dark:ring-white/10">
          ...
          <div class="py-1">
            <MenuItem v-slot="{ close }">
              <Link :href="editProfile().url" class="flex items-center gap-2 rounded-lg px-3 py-2 text-gray-700 hover:bg-gray-50 hover:text-gray-900 dark:text-gray-200 dark:hover:bg-white/5" @click="close">
                <Cog6ToothIcon class="size-4" aria-hidden="true" />
                Profile & Settings
              </Link>
            </MenuItem>
            <MenuItem>
              <button type="button" class="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-rose-600 hover:bg-rose-50 hover:text-rose-700 dark:text-rose-400 dark:hover:bg-rose-500/10" @click="handleLogout">
