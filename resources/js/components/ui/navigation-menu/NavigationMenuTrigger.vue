...
<template>
  <button ref="triggerRef" type="button" role="menuitem" :data-state="isOpen ? 'open' : 'closed'" :aria-expanded="isOpen" :class="cn(navigationMenuTriggerStyle(), 'group', props.class)" @click.prevent="onClick" @mouseenter="openOnHover" @focus="openOnHover">
    <span class="flex items-center gap-2">
      ...
      <Transition enter-active-class="transition-transform duration-200" leave-active-class="transition-transform duration-200">
        <svg v-if="props.chevron !== false" class="size-3 text-foreground/70 transition-transform duration-200 group-data-[state=open]:rotate-180" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
          <path d="M6 9l6 6 6-6" />
        </svg>
      </Transition>
    </span>
