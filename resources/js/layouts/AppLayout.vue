<script setup lang="ts">
  ...
  const navigation = computed<NavItem[]>(() => {
    ...
    return [{ title: "Dashboard", href: dashboardLink, isActive: current.startsWith(dashboardLink) }];
  });
</script>
  ...
  <div class="min-h-screen bg-gray-50 text-gray-900 dark:bg-gray-900 dark:text-gray-50">
    <TransitionRoot as="template" :show="sidebarOpen">
      <Dialog class="relative z-50 lg:hidden" @close="sidebarOpen = false">
        <TransitionChild as="template" enter="transition-opacity ease-linear duration-150" enter-from="opacity-0" enter-to="opacity-100" leave="transition-opacity ease-linear duration-150" leave-from="opacity-100" leave-to="opacity-0">
        ...
        <div class="fixed inset-0 flex">
          <TransitionChild as="template" enter="transition ease-in-out duration-200 transform" enter-from="-translate-x-full" enter-to="translate-x-0" leave="transition ease-in-out duration-200 transform" leave-from="translate-x-0" leave-to="-translate-x-full">
            <DialogPanel class="relative mr-10 flex w-full max-w-72 flex-1">
              <SideNav :brand="brand" :brand-href="dashboard().url" :mobile="true" :nav-items="navigation" :user="page.props.auth.user" class="w-full" @close="sidebarOpen = false" />
            </DialogPanel>
          </TransitionChild>
        </div>
      </Dialog>
    ...
    <TopNav class="lg:pl-72" :brand="brand" :brand-href="dashboard().url" :user="page.props.auth.user" @open-sidebar="sidebarOpen = true">
      <template #actions>
    ...
    <div class="flex">
      <aside class="hidden lg:fixed lg:inset-y-0 lg:z-40 lg:flex lg:w-72 lg:flex-col lg:border-r lg:border-white/10 lg:bg-gray-950 lg:pt-2" aria-label="Primary navigation">
